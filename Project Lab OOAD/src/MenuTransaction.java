...
public class MenuTransaction {
	...
	public MenuTransaction(int id, int quantity, String method, String date) {
		super();
		this.id = id;
	...
	public Vector<MenuTransaction>getAll(){
		Vector<MenuTransaction>menus=new Vector<>();
		try {
			...
			while(rs.next()) {
				...
				MenuTransaction menutransaction=new MenuTransaction(id,quantity,method,date);
				menus.add(menutransaction);
			}
			return menus;
