...
public class MenuTransactionManagement {
	...
	public boolean insert() {
		try {
			PreparedStatement ps=Connect.getInstance().prepareStatement("INSERT INTO transactionmanagement VALUES (null,?,?,?)");
	...
	public boolean update() {
		try {
			PreparedStatement ps=Connect.getInstance().prepareStatement("UPDATE transactionmanagement SET productquantity=?,productmethod=? WHERE productid=?");
			ps.setInt(1, quantity);
			ps.setString(2, method);
			ps.setString(3, date);
			ps.setInt(4, id);
