...
//		southPanel.add(cuisine);
	...
	private void refreshTable() {
		...
		Vector<MenuTransactionManagement>menus=MenuControllerTransactionManagement.getAllMenus();
		
		for(MenuTransactionManagement menutransactionmanagement: menus) {
			rowData=new Vector<>();
...
//			String query =String.format("INSERT INTO transactionmanagement VALUES(%d,%d,'%s','%s')",id,quantity,method,strDate);
			...
			if(MenuControllerTransactionManagement.updateMenu(id,quantity,method)) {
				...
				String errorMessage=MenuControllerTransactionManagement.errorMessage;
				JOptionPane.showMessageDialog(null, errorMessage,"Error",JOptionPane.ERROR_MESSAGE);
			}
//			try {
...
//			}
			...
			if(MenuControllerTransactionManagement.deleteMenu(id)){
				refreshTable();
